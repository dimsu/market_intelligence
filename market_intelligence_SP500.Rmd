...
kmeans_obj <- stock_date_matrix_tbl %>% 
    ...
    # Retrieve the symbol column 
    select(-symbol) %>% 
    
    # Apply the kmeans algorithm 4 centers and 20 groups
...
# Apply glance() to get the tot.withinss
    ...
    kmeans_obj %>% broom::glance() %>% select(tot.withinss)
...
# Visualize Scree Plot
 k_means_mapped_tbl %>% 
        ...
        theme_tq() + 
        labs(
            title = "Skree Plot of the SP500 companies subgroup (stock prices based)", 
            subtitle = "Author : Ralph D. Tasing", 
            caption = "Conclusion: Based on the skree plot, 10 subgroups are good enough to have a 
            representation of the SP500 base on stock prices. Behond 10 we have little difference") +
...
k_means_obj <-  k_means_mapped_tbl %>%
    # We pick 10 centers (centers = 10) to represent the data (skree plot) ----
    filter(centers == 10) %>%
        pull(k_means) %>%
        pluck(1)
